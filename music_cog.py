...
class music_cog(commands.Cog):
    ...
    async def play_music(self, ctx):
        if len(self.music_queue) > 0:
            ...
            if self.vc == None or not self.vc.is_connected():
                self.vs = await self.music_queue[0][1].connect()

                if self.vs == None:
                    await ctx.send("Could not connect to the voice channel")
            else:
                await self.vs.move_to(self.music_queue[0][1])

            self.music_queue.pop(0)
            self.vs.play(discord.FFmpegPCMAudio(m_url, **self.FFMPEG_OPTIONS), after=lambda e: self.play_next())
    ...
    @commands.command(name="resume", aliases=["r"], help = "Resumes playing the curent song")
